...
class MISECEController extends Controller
{
    ...
    function expece(Request $request){
        ...
        if (str_contains($response->body(), "no se encontr\u00f3 el paciente")) {
            ...
            return response()->json(['codemsg' => 'Un código de verificación ha sido enviado al paciente.'], 401);
        } else {
    ...
    private function GetObservation($reference, $status, $category, $values, $isbool)
    { //$category = Antecedentes || $code = dato de antecedentes (eg. Grupo Etnico) || $values[0] = valor, $values[1] = unidad
        $ob = new Observation;
